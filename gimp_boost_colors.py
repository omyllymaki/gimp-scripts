...
def boost_colors(image, drawable):
    ...
    gaussian_blur(active_layer, image, horizontal_radius=horizontal_radius, vertical_radius=vertical_radius)
    set_layer_opacity(active_layer, 35)

    layer_merged = merge_down(active_layer, image)
    set_grain_merge_mode(layer_merged)
    set_layer_opacity(layer_merged, 35)
